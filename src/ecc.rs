pub fn generate_ecc(data: &[u8], num_ecc_codewords: usize) -> Vec<u8> {
    let mut ecc = vec![0u8; num_ecc_codewords];
    ...
    ecc.copy_from_slice(&message[data.len()..]);
    ecc
}
...
fn get_generator_polynomial(degree: usize) -> Vec<u8> {
    ...
    for i in 0..degree {
        ...
        for j in 0..poly.len() {
            new_poly[j] ^= poly[j];
            new_poly[j + 1] ^= gf_multiply(poly[j], gf_pow(2, i));
        }
        poly = new_poly;
    }
...
fn gf_pow(base: u8, exp: usize) -> u8 {
    ...
    let log_base = GF_LOG[base as usize];
    let log_result = (log_base as usize * exp) % 255;
    
    GF_EXP[log_result]
}

// Galois Field lookup tables for GF(256)
const GF_EXP: [u8; 256] = [
